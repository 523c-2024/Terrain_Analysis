...
#mask second, so that you don't have more na's than you need
...
```{r}
#Create a point near the stream using lat, long. 
salmon_pt <- tibble(site = c('Salmon'),
                long = c(45.3902299),
                lat = c(-115.6029232)) %>%
  st_as_sf(., coords = c('lat','long'), crs = 4263) %>%
  st_transform(2163)
...
st_write(salmon_pt, 'data_salmon/salmon_point.shp', append = F)

# Use elevatr to download data and convert to terra object
salmon_elev <- get_elev_raster(salmon_pt, z = 9) %>%
  rast(.)
...
#Save local version for whitebox to use
writeRaster(salmon_elev, 'data_salmon/salmon_elev.tif', overwrite = T)

#Get a shillshade map from wbt
wbt_hillshade(dem = "data_salmon/salmon_elev.tif",
              output = "data_salmon/salmon_hillshade.tif",
              azimuth = 115)

#Read in the hillshade
salmon_hillshade <- rast('data_salmon/salmon_hillshade.tif')

#Visualize
tm_shape(salmon_hillshade)+
  tm_raster(style = "cont",
            palette = "-Greys", 
            legend.show = FALSE)+
  tm_scale_bar()
...
# Breach depressions (force water to move through ponds/lakes/etc...)
wbt_breach_depressions_least_cost(
  dem = "data_salmon/salmon_elev.tif",
  output = "data_salmon/salmon_breached.tif",
  dist = 9,
  fill = TRUE)
...
# Fill any remaining depressions
wbt_fill_depressions_wang_and_liu(
  dem = "data_salmon/salmon_breached.tif",
  output = "data_salmon/salmon_breachfill.tif"
)

#Get flow accumulation
wbt_d8_flow_accumulation(input = "data_salmon/salmon_breachfill.tif",
                         output = "data_salmon/salmon_d8fa.tif")
#Get flow direction
wbt_d8_pointer(dem ='data_salmon/salmon_breachfill.tif',
               output = 'data_salmon/salmon_d8point.tif')

# read in the data
salmon_fa <- rast('data_salmon/salmon_d8fa.tif') %>%
  log10(.)
...
#Extract streams at 1km2 
wbt_extract_streams(flow_accum = "data_salmon/salmon_d8fa.tif",
                    output = "data_salmon/salmon_raster_streams.tif",
                    threshold = 300)

#Snap our watershed flowliens to this point. 
wbt_jenson_snap_pour_points(pour_pts = "data_salmon/salmon_point.shp",
                            streams = "data_salmon/salmon_raster_streams.tif",
                            output = "data_salmon/salmon_snappedpp.shp",
                            snap_dist = 1000)

# Read in and check the snap. 
salmon_snap <- st_read('data_salmon/salmon_snappedpp.shp')
mapview(salmon_fa) + 
  mapview(salmon_snap)

wbt_watershed(d8_pntr = "data_salmon/salmon_d8point.tif",
              pour_pts = "data_salmon/salmon_snappedpp.shp",
              output = "data_salmon/southfork_salmon.tif")

southfork_salmon <- rast("data_salmon/southfork_salmon.tif")


mapview(southfork_salmon) + 
mapview(salmon_pt)

#First we need slope
wbt_slope(dem = 'data_salmon/salmon_elev.tif',
          output = 'data_salmon/salmon_slope.tif',
          units = 'degrees')

# Now we can generate twi
wbt_wetness_index(sca = 'data_salmon/salmon_d8fa.tif',
                  slope = 'data_salmon/salmon_slope.tif',
                  output = 'data_salmon/salmon_twi.tif')

salmon_twi <- rast('data_salmon/salmon_twi.tif')

mapview(salmon_twi)

# Crop TWI to south_shed area
#crop first
twi_salmon <- crop(salmon_twi, southfork_salmon)

#mask second, so that you don't have more na's than you need
twi_salmon_mask <- mask(twi_salmon, southfork_salmon)


mapview(twi_salmon_mask)


global(twi_salmon_mask, fun = 'mean', na.rm = T)
```
...
# Generate Aspect
wbt_aspect(dem = 'data_salmon/salmon_elev.tif',
           output = 'data_salmon/salmon_aspect.tif')

# Read Slope and Aspect data
salmon_slope <- rast('data_salmon/salmon_slope.tif')
salmon_aspect <- rast('data_salmon/salmon_aspect.tif')

# Visualize Slope
mapview(salmon_slope)

# Visualize Aspect
mapview(salmon_aspect)

# Crop Slope and Aspect to the southfork_salmon area
slope_salmon <- crop(salmon_slope, southfork_salmon)
aspect_salmon <- crop(salmon_aspect, southfork_salmon)

# Mask Slope and Aspect
slope_salmon_mask <- mask(slope_salmon, southfork_salmon)
aspect_salmon_mask <- mask(aspect_salmon, southfork_salmon)

# Visualize cropped and masked Slope
mapview(slope_salmon_mask)

# Visualize cropped and masked Aspect
mapview(aspect_salmon_mask)

# Extract statistics (e.g., mean) from Slope and Aspect
slope_stats <- global(slope_salmon_mask, fun = 'mean', na.rm = TRUE)
aspect_stats <- global(aspect_salmon_mask, fun = 'mean', na.rm = TRUE)
...
I pulled slope, which measures steepness, and aspect, which shows the
direction the slop is oriented, and cropped to my sub basin of the
Salmon River.
